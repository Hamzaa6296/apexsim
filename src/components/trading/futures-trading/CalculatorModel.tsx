          ...
          <div className="flex gap-6">
            {(["Profit/ loss", "Target Price", "Liquidation price"] as CalcTab[]).map((tab) => (
              <div
                ...
                className={`pb-2 text-[13px] font-semibold cursor-pointer transition-all relative z-10 ${activeTab === tab ? 'text-white' : 'text-gray-500 hover:text-gray-300'
                  }`}
              >
                {tab}
                {activeTab === tab && (
                  <div className="absolute -bottom-pxft-0 w-full h-[h-0.5-[#00B595] shadow-[0_0_8px_rgba(0,181,149,0.4)]" />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="p-4 space-y-5">
          ...
          <div className="flex bg-[#1e2023] rounded-md overflow-hidden ">
            <button
              ...
              className={`flex-1 py-2 text-sm rounded cursor-pointer transition-all ${side === 'long' ? 'bg-[#0ECB81] text-white' : 'text-gray-400 '
                }`}
            >Open Long</button>
            <button
              onClick={() => setSide('short')}
              className={`flex-1 py-2 text-sm  rounded cursor-pointer transition-all ${side === 'short' ? 'bg-[#0ECB81] text-white' : 'text-gray-400'
                }`}
            >Open Short</button>
          </div>
          ...
          <div className="pt-2">
            <div className="relative h-1 flex items-center mb-6">
              ...
              <div className="absolute left-0 w-0 h-0.5g-[#00B595]" />
