...
export default function BotMarketForm() {
    ...
    const aiStrategies = [
        { title: "Short-term fluctuation-7D", apr: "+251.467%", range: "$88,200.84-$88,967", grids: 6, fee: "0.13%-0.20%" },
        { title: "Mid-term sideways-30D", apr: "+251.467%", range: "$88,200.84-$88,967", grids: 6, fee: "0.13%-0.20%" }
    ];

    const radioOptions = ["Trailing settings", "TP and SL", "Sell all base coins on stops"];
            ...
            <div className="flex px-4 pt-2 gap-6 border-b border-white/5 relative">
                {(["AI", "Popular", "Manual"] as TabType[]).map((tab) => (
                    ...
                    >
                        {tab}
                        {activeTab === tab && (
                            <div className="absolute bottom-0 left-0 w-full h-[3px] bg-[#00B595] rounded-t-full" />
                        )}
                    </div>
                ))}
            </div>
                ...
                {activeTab === "AI" && (
                    <div className="space-y-4">
                        {aiStrategies.map((strat, idx) => (
                            <div key={idx} className="bg-[#1C1C1D] p-4 flex flex-col gap-4 border border-transparent">
                                <h3 className="text-[18px] border-b border-b-gray-800 font-semibold text-gray-200 pb-2">{strat.title}</h3>
                                <div className="grid grid-cols-2">
                                    <div>
                                        <p className="text-[15px] text-gray-500">Backtested APR</p>
                                        <p className="text-[#00B595] text-2xl font-bold">{strat.apr}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[15px] text-gray-500">Profit per grid</p>
                                        <p className="text-[15px] text-gray-500 font-bold">{strat.fee}</p>
                                    </div>
                                    <div className="mt-2">
                                        <p className="text-[15px] text-gray-500">Price range (USDT)</p>
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="space-y-1.5">
                                ...
                                <input placeholder="Lower" className="w-full bg-[#1C1C1D] mt-2 border border-white/10 rounded p-2.5 text-sm outline-none" />
                            </div>
                            <div className="space-y-1.5">
                                <label className="text-[15px] text-gray-50">Price range</label>
                                <input placeholder="Upper" className="w-full bg-[#1C1C1D] mt-2 border border-white/10 rounded p-2.5 text-sm outline-none" />
                            </div>
                        </div>

                        <div>
                            <label className="text-[15px] text-gray-50">The number of grids</label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[15px] text-gray-50">Investment amount</label>
                            <div className="relative">
                                <input placeholder='50.00' className="w-full bg-[#1C1C1D] border mt-2 border-white/10 rounded p-2.5 text-sm outline-none" />
                                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                    <span className="text-[13px] text-gray-50 pl-2 border-l border-white/10">USDT</span>
                                ...
                                {radioOptions.map((option) => (
                                    ...
                                    >
                                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${
                                            selectedOption === option ? "border-[#0052FF]" : "border-gray-600"
                                        }`}>
                                            {selectedOption === option && (
                                                <div className="w-2 h-2 rounded-full bg-[#0052FF]" />
                                            )}
                                        </div>
                                        <span className={`text-[13px] ${selectedOption === option ? "text-gray-200" : "text-gray-400"}`}>
                                            {option}
                                        </span>
                                    </div>
                                ))}
                        ...
                        <button className="w-full py-3 bg-gradient-to-r from-[#0052FF] to-[#0070FF] text-white rounded-md cursor-pointer transition-all active:scale-[0.98]">
                            Create
                        </button>
                    </div>
                )}
